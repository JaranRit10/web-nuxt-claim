...
context('User Setting Menu', () => {
  ...
  describe('User Setting Create', () => {
    it('Open page', () => {
      ...
      // cy.get('.page__title')
      // .should('have.text', 'User Management Create')

      // cy.url()
      // .should('include', '/setting/user/create')
    })
  })
  ...
  describe('User Setting Search', () => {
    ...
    it('Search by criteria', () => {
      ...
      cy.get('.a-table table tbody tr td:nth-child(6)')
        .each(($el, index, $list) => {
          cy.wrap($el)
            .contains('Active')
    ...
    it('Search after clear', () => {
      ...
      cy.get('.a-table table tbody tr')
        .should('have.length', found) // search all must found rows more than serch by criteria
    })
  })
