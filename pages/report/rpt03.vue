<template>
  <div>
    <HasPerm any="report03_export">
      <!-- Page Header -->
      <div class="page__header">
        <div class="page__title">
          <h1>{{ $t('report') }}</h1> <span>{{ $t('policy_transaction') }}</span>
        </div>
        <div class="page__breadcrumb">
          <i class="las la-md la-home"></i> {{ $t('c.home') }}  / {{ $t('report') }} / {{ $t('policy_transaction') }}
        </div>
      </div>
      <!-- Search Criteria -->
      <div class="card">
...
export default {
  ...
  methods: {
    ...
    // },
    exportFile () {
      let reportTest = 'RPT003_POLICY_TRANSACTION_REPORT.rptdesign'
